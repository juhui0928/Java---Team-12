...
public class MatchingService {
    ...
    // 💡 [핵심 추가] 중복된 학번의 기존 데이터를 새 정보로 덮어쓰고 파일을 새로 저장하는 메서드
    public void updateUserAndFile(User updatedUser) throws IOException {
        ...
        // 2. 파일 전체를 새로 쓰기(Overwrite)하여 동기화
    ...
    public List<String> getMatchResults(User currentUser) {
        ...
        for (User other : userList) {
            ...
            if (!other.getGender().equals(currentUser.getGender())) continue;

            Preference o = other.getPreference();
            
            int score = Math.abs(my.getSmoking() - o.getSmoking()) * 20
                      + Math.abs(my.getDrinking() - o.getDrinking()) * 15
                      + Math.abs(my.getSleep() - o.getSleep()) * 10
                      + Math.abs(my.getCleaning() - o.getCleaning()) * 7
                      + Math.abs(my.getNoise() - o.getNoise()) * 7
                      + Math.abs(my.getCall() - o.getCall()) * 4
                      + Math.abs(my.getEating() - o.getEating()) * 2;
